...
##Method 1 uses wallShearStress computed from OpenFOAM directly for computing friction velocity required for wall units
...
##The location of wallshearstress file to be used. 
fwallshear= open("/home/bhanu/airfoil/66.75445/wallShearStress", "r")
...
for i in range(len(cells)):
	mag.append(math.sqrt( math.pow(wallgradU[i][0],2) + math.pow(wallgradU[i][1],2) + math.pow(wallgradU[i][2],2)))

density = 1
frictionvelocity = [] 
for i in range(len(cells)):
	frictionvelocity.append(math.sqrt(mag[i]/density))
